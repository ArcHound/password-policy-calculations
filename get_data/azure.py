...
class AzureScraper:
    def __init__(self, proxy, proxies, cards, base_doc_url="https://learn.microsoft.com", base_api_url="https://prices.azure.com", cache_name="azure_cache", backend="sqlite"):
        log.info("Init azure session...")
        self.base_url = base_doc_url
        self.api_url = base_api_url
        self.cards = cards
        self.azure_session = CachedSession(cache_name=cache_name, backend=backend, cache_control=True,  expire_after=timedelta(days=1))
        self.azure_api_session = CachedSession(cache_name=cache_name+'_api', backend=backend, cache_control=False,  expire_after=timedelta(days=1))
    ...
    def crawl(self):
        ...
        for x in self.pricing_reqs:
            try:
                sku = x['sku']
                prices = self.azure_api_session.get(self.api_url+'/api/retail/prices', params={"$filter":f"armSkuName eq '{sku}' and priceType eq 'Consumption'"}).json()
    ...
    def parse_pricing(self, pricing_data):
        less_data = [x for x in pricing_data["Items"] if 'effectiveEndDate' not in x] # exclude old offers
        return min(less_data,key=lambda x: x['unitPrice']) # hopefully they're all billed by the hour 
